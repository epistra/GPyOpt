...
class DPP(EvaluatorBase):
    ...
    def sample_base_points(self, N_points=None, context_manager=None):
        ...
        if self.base_points is None:
            if not self.suppress_emb and hasattr(kern, "emb_min") and hasattr(kern, "emb_max") and hasattr(kern, "transform_pinv"):
                # if kernel is LinEmbKern, base_points are placed in reduced subspace
                domain_emb = [
                    {'name': 'var_{}'.format(i), 'type': 'continuous', 'domain': (kern.emb_min[i], kern.emb_max[i])}
                    for i in range(kern.emb_dim)
                ]
                space_emb = Design_space(domain_emb, None)
                base_points_emb = initial_design(self.design_name, space_emb, N_points)

                # transform base_points_emb to base_points
                base_points = kern._low2high_transform(base_points_emb, randomize=self.randomize)

            elif not self.suppress_emb and context_manager and context_manager.A_reduce is not None:
    ...
    def compute_batch(self, duplicate_manager=None, context_manager=None, batch_context_manager=None):
        ...
        while True:
            # sample base points
            base_points = self.sample_base_points(N_points, context_manager)
